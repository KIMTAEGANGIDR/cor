...
def main():
    ...
    for i, (lt, nom, thn, bp, lp, bs, ls) in enumerate(pairs):
        ...
        bp_pg, bp_tx, bp_err = analyze_pdf(bp)
        lp_pg, lp_tx, lp_err = analyze_pdf(lp)
    ...
    print(f"\r   {total:,}/{total:,} (100%)", flush=True)
    ...
    # 손상된 파일
    stats['corrupted'] = [
        {'law_type': r['law_type'], 'nomor': r['nomor'], 'tahun': r['tahun'], 'error': r['leg_err']}
        for r in results if r['leg_err']
    ]
    ...
    for lt in LAW_TYPES.keys():
        s = stats[lt]
        print(f"{lt:<8} {s['total']:>6,} {s['same_pages']:>6,} {s['bpk_more']:>6,} {s['leg_more']:>7,} {s['leg_err']:>5,}", flush=True)

    t = stats['TOTAL']
    print("-" * 60, flush=True)
    print(f"{'합계':<8} {t['total']:>6,} {t['same_pages']:>6,} {t['bpk_more']:>6,} {t['leg_more']:>7,} {t['leg_err']:>5,}", flush=True)
    ...
    print(f"  법제처 PDF 손상: {t['leg_err']:,}개", flush=True)
    ...
    if anomalies:
        print(f"\n■ 이상 케이스 (페이지 10+ 차이) TOP 10", flush=True)
    ...
    print(f"\n완료! ({(end-start).total_seconds():.0f}초)", flush=True)
