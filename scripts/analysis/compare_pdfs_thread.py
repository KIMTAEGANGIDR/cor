...
def analyze_with_timeout(path, timeout=TIMEOUT_SEC):
    """타임아웃 포함 PDF 분석"""
    with ThreadPoolExecutor(max_workers=1) as executor:
        future = executor.submit(analyze_pdf_sync, path)
        try:
...
def main():
    ...
    for i, (lt, nom, thn, bp, lp, bs, ls) in enumerate(pairs):
        if i % 50 == 0:
            ...
            print(f"   {i:,}/{total:,} ({i*100//total}%) | 타임아웃:{timeouts} 에러:{errors} | ETA:{eta:.0f}분", flush=True)

        bp_pg, bp_tx, bp_err = analyze_with_timeout(bp)
        lp_pg, lp_tx, lp_err = analyze_with_timeout(lp)
    ...
    print(f"   {total:,}/{total:,} (100%) | 타임아웃:{timeouts} 에러:{errors}", flush=True)
